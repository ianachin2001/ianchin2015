 ...
 * PIDOutput
 */
public class PIDMotorController implements LiveWindowSendable{
	
	double speedToEndAt = 0;
	double distanceToSlowDownAt = 0;
	boolean slowDown = false;
	public CANTalon talon_to_control;
    ...
    private double max_deccel = 1;
    private double max_speed = 1;
    private double speed = 0;
    private double accel = 0;
    double distanceToMove = 0;
    boolean firstAfterSetPoint = true;
    /**
     ...
     */
    public PIDMotorController(double Kp, double Ki, double Kd, double Kf,
                         PIDSource source, PIDOutput output,
                         double period,CANTalon talon) {
     ...
     * integral and differential terms. The default is 50ms.
     */
    public PIDMotorController(double Kp, double Ki, double Kd,
                         PIDSource source, PIDOutput output,
                         double period) {
    ...
    private void calculate() {
        ...
        if(enabled){
        	...
        	synchronized(this){
        		...
        		if(Math.sqrt(speedToEndAt*speedToEndAt + 2*(max_deccel *(m_setpoint-input)))<= max_speed && m_setpoint > input){
        			slowDown = true;
        		}
        		else if(Math.sqrt(speedToEndAt*speedToEndAt + 2*(max_deccel *(input - m_setpoint)))<= max_speed && m_setpoint < input){
        			slowDown = true;
        		}
        		if(input >= distanceToSlowDownAt){
        			slowDown = true;
        		}
        		if(firstAfterSetPoint){
        			distanceToMove = m_setpoint - input;
        			firstAfterSetPoint = false;
        		}
        		if(distanceToMove != 0){
        			if(speed+max_accel*20 <= max_speed && slowDown == false){
        				speed = speed+max_accel*20;
        				talon_to_control.set(input + speed);
        			}		
        			else if(speed + max_accel*20 > max_speed && slowDown == false){
        				speed = max_speed;
        				talon_to_control.set(input + speed);
        			}
        			else if(slowDown && speed - max_deccel*20 > 0){
        				speed = speed - max_deccel*20;
        				talon_to_control.set(input + speed);
        			}
        			else if(slowDown && speed - max_deccel*20 <= 0){
        				if(distanceToMove > 0){
        					speed = m_setpoint - input;
        				}
        				else if(distanceToMove < 0){
        					speed = input - m_setpoint;
        				}
        				else{
        					speed = 0;
        				}
        				talon_to_control.set(m_setpoint);
        			}
    				if(distanceToMove > 0){
    					talon_to_control.set(input + speed);
    				}
    ...
    public synchronized void setMaxSpeed(double encoderTicksPerMillisec){
    	max_speed = encoderTicksPerMillisec;
    }
    public synchronized void setMaxAccel(double encoderTicksPerMillisecPerMillisec){
    	max_accel = encoderTicksPerMillisecPerMillisec;
    }
    public synchronized void setMaxDeccel(double encoderTicksPerMillisecLostPerMillisec){
    	max_deccel = encoderTicksPerMillisecLostPerMillisec;
    }
    /**
    ...
    */
     ...
     */
    public synchronized void setSpeedToEndAt(double speedToEndAt){
    	this.speedToEndAt = speedToEndAt;
    }

    public synchronized void setSetpoint(double setpoint) {
    	slowDown = false;
    	firstAfterSetPoint = true;
